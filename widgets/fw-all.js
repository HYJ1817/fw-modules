...
async function loadDetail(link){var s=String(link||"").replace(/^missav:/i,"");var u=s.indexOf("http")===0?s:B+"/"+s;try{var r=await Widget.http.get(u,{headers:{"User-Agent":U,Referer:B+"/cn"},timeout:15000}),h=typeof r?.data==="string"?r.data:(r?.body||"");var tm=h.match(/<title[^>]*>([\s\S]*?)<\/title>/i),title=tx(tm?tm[1]:s).replace(/\s+-\s+MissAV.*$/i,"");var pm=h.match(/data-poster=["']([^"']+)/i),poster=pm?pm[1]:"";return {id:"missav:"+s,type:"url",title:title,seriesName:title,description:title,posterPath:poster,backdropPath:poster,mediaType:"movie",link:"missav:"+s};}catch(e){return {id:"missav:"+s,type:"url",title:s,seriesName:s,mediaType:"movie",link:"missav:"+s};}}
