...
function parse(h){let a=[],re=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi,m;while(m=re.exec(h||"")){let u=m[1],t=tx(m[2]);if(/\/(?:cn\/)?[A-Za-z0-9]+-[0-9]+|\/cn\/[A-Za-z0-9]+/i.test(u)&&t&&t.length>3&&u.indexOf("#")<0){if(u[0]==="/")u=B+u;if(!a.some(x=>x.url===u))a.push({url:u,title:t,posterPath:(m[2].match(/<img[^>]+(?:src|data-src)=["']([^"']+)/i)||[])[1]||""})}}return a}
...
function out(a){return a.map(function(x){var slug=x.url.split("missav.live/")[1]||x.url;return {id:"missav:"+slug,type:"url",title:x.title,seriesName:x.title,description:x.title,mediaType:"movie",posterPath:x.posterPath,link:"missav:"+slug};})}
