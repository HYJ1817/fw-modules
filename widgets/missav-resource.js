...
async function loadResource(p){p=p||{};let raw=String(p.link||p.url||p.id||"").replace(/^missav:/i,"");let u=raw;if(!/^https?:/i.test(u))u=BASE+(u.startsWith("/")?u:"/"+u);try{let r=await Widget.http.get(u,{headers:{"User-Agent":UA,Referer:BASE+"/cn"},timeout:15000}),h=html(r),xs=media(h);return xs.map((x,i)=>({id:"missav:"+x,name:"MissAV"+(i?" · 备用":""),description:title(h.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]||u),url:x,customHeaders:{"User-Agent":UA,Referer:u},playerType:"app"}))}catch(e){return[]}}
