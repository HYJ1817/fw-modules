...
function abs(u){u=String(u||"");if(u.startsWith("//"))return "https:"+u;if(u.startsWith("/"))return B+u;return u}
function parse(h){let a=[],re=/<a\b[^>]*href=["'](\/cn\/[A-Za-z0-9]+(?:-[A-Za-z0-9]+)*)["'][^>]*>([\s\S]*?)<\/a>/gi,m;while(m=re.exec(h||"")){let u=B+m[1],block=m[2],img=block.match(/<(?:img|video)\b[^>]*(?:data-src|src)=["']([^"']+)["'][^>]*>/i),alt=block.match(/\balt=["']([^"']*)["']/i),t=tx(alt?alt[1]:block);if(!t||t.length<2||a.some(x=>x.url===u))continue;a.push({url:u,title:t,posterPath:img?abs(img[1]):""})}return a}
...
function out(a){return a.map(function(x){var slug=x.url.split("missav.live/")[1];return {id:"missav:"+slug,type:"url",title:x.title,seriesName:x.title,description:x.title,mediaType:"movie",posterPath:x.posterPath,link:"missav:"+slug};})}
